...
const iconsvgClass =
  "h-7 w-7 transition-default hover:h-9 hover:w-9 cursor-pointer";
...
export default function MenuCard({ item, className = "", ...props }) {
  ...
  const isEmail =
    item.id === "email" ||
    (typeof item.slug === "string" && item.slug.includes("@"));
  ...
  return (
    ...
    >
      ...
      <a
        href={linkHref}
        className="rounded-full bg-accent text-bg flex items-center justify-center p-[var(--spacing-md)] mb-[var(--spacing-sm)]"
      >
